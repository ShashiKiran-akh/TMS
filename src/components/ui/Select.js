...
export const Select = forwardRef(({ label, error, fullWidth = false, options, className = '', ...props }, ref) => {
    return (_jsxs("div", { style: { marginBottom: 'var(--spacing-2)', width: fullWidth ? '100%' : 'auto' }, children: [label && (_jsx("label", { style: {
                ...
                }, children: label })), _jsx("select", { ref: ref, style: {
                    width: '100%',
                    padding: '10px 12px',
                    fontSize: '1rem',
                ...
                }, className: className, ...props, children: options.map((option) => (_jsx("option", { value: option.value, children: option.label }, option.value))) }), error && (_jsx("p", { style: {
                    marginTop: '4px',
                    fontSize: '0.875rem',
